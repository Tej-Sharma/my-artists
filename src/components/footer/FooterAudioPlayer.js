...
class FooterAudioPlayer extends Component {
    ...
    getSpotifyPlayer() {
        if (window.Spotify !== null) {
            ...
            this.player = new window.Spotify.Player({
              name: "My Artists",
              getOAuthToken: cb => { cb(this.state.aToken); },
            });
    ...
    createPlayerEventHandlers() {
        ...
        this.player.on('player_state_changed', newState => this.playerStateChanged(newState));
       ...
       */
      playHere(devId) {
        this.spotifyApi.transferMyPlayback([devId], { limit: 10 })
        .then(result => {
            console.log(result);
    ...
    render() {
        return this.state.loggedIn ? 
            <div className="audioFooter">
                <input id="playButton" type="image" alt="Login"
